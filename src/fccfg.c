...
#define _lock fontconfigBase->_lock
#define _fcConfig fontconfigBase->_fcConfig
#else
...
lock_config (void)
{
    ...
    FcMutexLock (lock);
#else
    _lock = IExec->AllocSysObject(ASOT_MUTEX, NULL);
    if (_lock) {
        IExec->MutexObtain(_lock);
        /* Initialize random state */
        FcRandom ();
    }
...
#else
    if (_lock) {
		IExec->FreeSysObject(ASOT_MUTEX, _lock);
		_lock = NULL;
    }
#endif
}
...
#else
#define _FcConfigHomeEnabled fontconfigBase->_FcConfigHomeEnabled
#endif
